...
-(void)play:(float)time {
    ...
    [self seekTime:time];
    [NHPlayTimer timerWithTimeInterval:1 / self.fps start:0 target:self action:@selector(displayNextFrame) repeats:YES async:NO onlyFlag:@"play"];
}

- (void)displayNextFrame {
    
    if (![self stepFrame]) {
        [NHPlayTimer cancelTask:@"play"];
