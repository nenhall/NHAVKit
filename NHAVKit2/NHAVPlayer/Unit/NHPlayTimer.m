...
#import "NHPlayTimer.h"
#include <objc/message.h>

static NSMutableDictionary *timers_bk;
dispatch_semaphore_t semaphore_bk;

@implementation NHPlayTimer

+ (void)initialize {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        timers_bk = [NSMutableDictionary dictionary];
        semaphore_bk = dispatch_semaphore_create(1);
    });
}
...
+ (NSString *)timerWithTimeInterval:(NSTimeInterval)interval
    ...
    dispatch_semaphore_wait(semaphore_bk, DISPATCH_TIME_FOREVER);
    [timers_bk setObject:source_t forKey:onlyFlag];
    dispatch_semaphore_signal(semaphore_bk);
