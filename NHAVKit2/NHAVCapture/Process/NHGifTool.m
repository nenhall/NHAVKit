        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            switch (assetExportSession.status) {
                ...
                case AVAssetExportSessionStatusUnknown: {
                    ...
                    kNSLog(@"Export Status: Unknown");
                }
                case AVAssetExportSessionStatusExporting : {
                    
                    kNSLog(@"Export Status: Exporting");
                }
                case AVAssetExportSessionStatusWaiting: {
                    
                    kNSLog(@"Export Status: Wating");
                }
            }
        });
    }];
...
- (NSURL *)createGIFforTimePoints:(NSArray *)timePoints fromURL:(NSURL *)url fileProperties:(NSDictionary *)fileProperties  frameProperties:(NSDictionary *)frameProperties gifImagePath:(NSString *)imagePath frameCount:(int)frameCount gifSize:(GIFSize)gifSize{
    ...
    for (NSValue *time in timePoints) {
        ...
        if (error) {
            ...
            _error =error;
            kNSLog(@"Error copying image: %@", error);
            return nil;
        ...
        if (imageRef) {
            ...
            kNSLog(@"Error copying image and no previous frames to duplicate");
            return nil;
        }
    ...
    if (!CGImageDestinationFinalize(destination)) {
        ...
        kNSLog(@"Failed to finalize GIF destination: %@", error);
        if (destination != nil) {
